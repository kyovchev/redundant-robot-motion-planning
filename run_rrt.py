...
startTime = time.time()
path, prev = rrt(START, END, OBSTACLE, delta=DELTA)
time = time.time() - startTime

ws = drawObstacle(ws, OBSTACLE)
wsPath = drawPathWithRobot(ws, path)

cv2.imwrite(f'{OUTPUT_DIR}/{FILE_PREFIX}rrt_{DELTA}.bmp', wsPath)

ws2 = drawWorkspaceTree(ws, prev)
cv2.imwrite(f'{OUTPUT_DIR}/{FILE_PREFIX}rrt_{DELTA}_tree.bmp', ws2)

animatePath(f'{FILE_PREFIX}rrt_{DELTA}', ws, path, 10)

getAllMetrics(f'{FILE_PREFIX}rrt_{DELTA}', path, prev, time)
